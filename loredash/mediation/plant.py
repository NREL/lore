...
plant_design = {
    ...
    'T_htf_hot_des':                574.0,          # C
    'P_ref':                        115,            # MW
    'gross_net_conversion_factor':  0.90,
    'design_eff':                   0.412,
    'Qrec':                         669.9,          # MWt
    'tshours':                      10.1,           # hr (TODO: using 10 hrs, SSC hangs during simulation??)  
    ...
    'rec_height':                   21.6029,          # Heated panel length (m)
    'D_rec':                        17.65,          # m
    'd_tube_out':                   40,           # mm
    'th_tube':                      1.25,          # mm
    'N_panels':                     20,
    'h_tower':                      193.458,            # m
    'mat_tube':                     2,             # 2 = SS316, 32 = H230, 33 = 740H
    ...
    'T_rout_min':                   275.,           # Minimum receiver HTF outlet T (C)  assume 15 C  less than design point cold tank temperature
    'T_rout_max':                   574.,           # Maximum receiver HTF outlet T (C)
    # --operational properties and losses
    'f_rec_min':                    0.25, 	        # TODO: Minimum receiver turndown ratio
    'csp_pt_rec_max_oper_frac':     1.2,            # TODO: Maximum receiver mass flow rate fraction  
    'piping_loss':                  10200, 	        # TODO: Need a better estimate of piping loss, was set to zero for model validation cases as outlet T measurements were before downcomer
    ...
    'min_preheat_time':             36./60,         # Median Preheat time 
    'min_fill_time':                10./60,         # Median fill time
    'startup_ramp_time':            16./60,         # Combined Operate and Track (to near 100% or point in time tracking stops changing)
    # --startup parameters for time/energy startup model.  Note that these values won't be used if the transient startup modeled is enabled
    'rec_qf_delay':                 0.25,           # Energy-based receiver startup delay (fraction of rated thermal power).
    'rec_su_delay':                 0.20,         # Fixed receiver startup delay time (hr). 

    # TES
    'h_tank':                       12.0,           # m  # Tank height (m)
    'h_tank_min':                   1.0,            # m  # Minimum allowable HTF height in storage tank (m)
    'hot_tank_Thtr':                500,            # C
    'cold_tank_Thtr':               280,            # C       
    ...
    'T_hs_max':                     574.,           # Maximum HTF T in hot storage (C)
    'T_cs_min':                     290.,           # Minimum HTF T in cold storage (C)
    ...
    'ud_f_W_dot_cool_des':          0.0,        # This is an estimate for the cooling system (700 kW) TODO: update user-defined cycle data with ambient variation
    'ud_m_dot_water_cool_des':      0.0,
    'ud_ind_od':                    [[0]],
    'tech_type':                    1,              # 1=fixed, 3=sliding
    'P_boil':                       100,            # Boiler operating pressure [bar]
    'CT':                           2,              # 1=evaporative, 2=air*, 3=hybrid
    'T_amb_des':                    42.0,           #58   # C
    'P_cond_min':                   2.0,            #3.0  # inHg
    'is_elec_heat_dur_off':         True,           # Use cycle electric heater parasitic when the cycle is off
    'W_off_heat_frac':              0.05,          # Electric heaters when cycle is off state (CD data) [Accounts for Startup and Aux Boiler Electric Heaters (~5.4 MWe)]
    'pb_pump_coef':                 0.55,           # HTF pumping power through power block (kWe / kg/s) (CD data)   [Accounts for Hot Pumps]  
    'pb_fixed_par':                 0.0055,       # Constant losses in system, includes ACC power (CD data) -- [Accounts for SGS Heat Trace, WCT Fans & Pumps, Cond Pump, SGS Recir Pumps, Compressors]
    'aux_par':                      0.023,            # Aux heater, boiler parasitic - Off"           
    'bop_par':                      0.0,          # Balance of plant parasitic power fraction ----- [Accounts for Feedwater pump power]
    'bop_par_f':                    1,              # Balance of plant parasitic power fraction - mult frac
    'bop_par_0':                    0,              # Balance of plant parasitic power fraction - const coeff
    'bop_par_1':                    0.483,    # Balance of plant parasitic power fraction - linear coeff
    'bop_par_2':                    0.,     # Balance of plant parasitic power fraction - quadratic coeff
    'startup_frac':                 0.5,            # Fraction of design thermal power needed for cold startup
    'startup_time':                 0.5,            # Time needed for power block startup (hr)
    'cycle_cutoff_frac':            0.2,            # TODO: Minimum cycle fraction. Updated from 0.1 used in model validation to 0.3 (minimum point in user-defined pc file)
    'cycle_max_frac':               1.05,        
    ...
    'Pc':                           [0.550],             # Slope of the HTF pumping power through the cycle regression region i [kWe / kg/s]
    'Bc':                           [0.0],     # Intercept of the HTF pumping power through the cycle regression region i [kWe / kg/s]
    'Pfw':                          [0.0],             # Slope of the feed water pumping power through the cycle regression region i [kWe / kg/s]
    'Bfw':                          [0.0],      # Intercept of the feed water pumping power through the cycle regression region i [kWe / kg/s] 
