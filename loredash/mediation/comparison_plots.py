...
def plot_solution(
    ...
    nrow = 8
    [fig, ax, nrow, ncol] = setup_subplots(
        nrow=nrow,
        ncol=1,
        wsub=3.5 * nday,
        hsub=0.75,
    ...
    # ==========================================================================
    #   Solar field adjustment
    # e.g. soiling if not using CD data, or soiling/tracking/offline if using
    # CD data
    j+=1
    ax[j].plot(
        times,
        tech_outputs['sf_adjust_out'],
        lw=0.75,
        color='grey',
        label='sf_adjust',
    )
    ...
    ax[j].plot(
        times,
        np.array(dispatch_soln['Rdisp']['disp_receiver_power'])/1000,
        '--',
        lw=0.75,
        color='maroon',
        label='Dispatch',
    )
    ax[j].set_ylabel(NAME_MAP['Q_thermal'])
    ...
    ax2.fill_between(
        times,
        dispatch_soln['Rdisp']['disp_receiver_standby'],
        lw=0.75,
        color='maroon',
        alpha=0.3,
        label='Standby',
    ...
    ax2.fill_between(
        times,
        dispatch_soln['Rdisp']['disp_receiver_startup'],
        lw=0.75,
        color='darkgreen',
        alpha=0.3,
        label='Startup',
    ...
    ax[j].plot(
        times,
        np.array(np.array(dispatch_soln['Rdisp']['disp_thermal_input_to_cycle']))/1000,
        '--',
        lw=0.75,
        color='maroon',
        label='Dispatch',
    ...
    ax2.fill_between(
        times,
        [1 if dispatch_soln['Rdisp']['disp_thermal_input_to_cycle'][idx] > 1e-6 else 0 for idx in range(npts)],
        lw=0.75,
        color='grey',
        alpha=0.3,
        label='On',
    ...
    ax2.fill_between(
        times,
        dispatch_soln['Rdisp']['disp_cycle_startup'],
        lw=0.75,
        color='darkgreen',
        alpha=0.3,
        label='Startup',
    ...
    ax2.fill_between(
        times,
        dispatch_soln['Rdisp']['disp_cycle_standby'],
        lw=0.75,
        color='maroon',
        alpha=0.3,
        label='Standby',
    ...
    ax[j].plot(
        times,
        np.array(dispatch_soln['Rdisp']['disp_electrical_output_from_cycle'])/1000,
        '--',
        lw=0.75,
        color='maroon',
        label='Dispatch (gross)',
    )
    ax[j].plot(
        times,
        np.array(dispatch_soln['Rdisp']['disp_net_electrical_output'])/1000,
        '--',
        lw=0.75,
        color='darkgoldenrod',
        label='Dispatch (net)',
    ...
    ax[j].plot(
        times,
        np.array(dispatch_soln['Rdisp']['disp_tes_soc'])/1000,
        '--',
        lw=0.75,
        color='maroon',
        label='Dispatch',
    ...
    ax[j].set_ylim([0, 1.05E-3*(dispatch_soln['Rdisp']['disp_Eu'])])
    ax[j].legend(loc='lower left')
    # ==========================================================================
    #   Receiver outlet T
    j += 1
    ax[j].plot(
        times,
        tech_outputs['T_rec_out'],
        lw=0.75,
        color='steelblue',
        label='SSC')
    ax[j].plot(
        times,
        np.array(dispatch_soln['Rdisp']['disp_receiver_outlet_temp']),
        '--',
        lw=0.75,
        color='maroon',
        label='Dispatch',
    ...
    ax[j].plot(
        times,
        np.array(dispatch_soln['Rdisp']['disp_hot_salt_tank_temp']),
        '--',
        lw=0.75,
        color='maroon',
        label='Dispatch',
    ...
    ax2.plot(
        times,
        np.array(dispatch_soln['Rdisp']['disp_cold_salt_tank_temp']),
        '--',
        lw=0.75,
        color='darkgoldenrod',
        label='Dispatch',
