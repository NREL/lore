...
def main(dates = ALL_DATES, log_filename = "logs.json", max_iter = 35):
    ...
    x0s = np.random.uniform(upper, lower, (3, 5))

    i = 0
    for x0 in x0s:
        print(x0)
        log_f = "./output_files/per_day/%s" % ("DFO_35RR%d_%s" % (i, log_filename))
        ...
        # print(soln1)
        soln1.diagnostic_info.to_csv(log_f)
        i += 1
