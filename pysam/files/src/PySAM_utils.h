...
//
static char* DAO_TK_SAM_lib_dir = NULL;	// dir ends with '/'
...
static void* DAO_TK_SAM_lib_handle = NULL;

static int PySAM_load_lib(PyObject* m){
    if (!DAO_TK_SAM_lib_path){
		...
		DAO_TK_SAM_lib_dir = malloc(strlen(filename) - strlen(lastSlash) + 2);
		memcpy(DAO_TK_SAM_lib_dir, filename, strlen(filename) - strlen(lastSlash) + 1);
		DAO_TK_SAM_lib_dir[strlen(filename) - strlen(lastSlash) + 1] = '\0';

        DAO_TK_SAM_lib_path = malloc(strlen(DAO_TK_SAM_lib_dir) + strlen(SAM_lib) + 1);

		memcpy(DAO_TK_SAM_lib_path, DAO_TK_SAM_lib_dir, strlen(DAO_TK_SAM_lib_dir) + 1);
        strcat(DAO_TK_SAM_lib_path, SAM_lib);

        PyObject *sys_path = PySys_GetObject("path");
        PyList_Append(sys_path, PyUnicode_FromString(DAO_TK_SAM_lib_dir));
...
static int PySAM_assign_from_dict(void *data_ptr, PyObject *dict, const char *tech, const char *group){
    ...
    while (PyDict_Next(dict, &pos, &key, &value)){
        ...
        if (PyNumber_Check(value)){
            SAM_error error = new_error();
            SAM_set_double_t func = SAM_set_double_func(DAO_TK_SAM_lib_handle, tech, group, name, &error);
        ...
        // string
            ...
            SAM_error error = new_error();
            SAM_set_string_t func = SAM_set_string_func(DAO_TK_SAM_lib_handle, tech, group, name, &error);
        ...
        // sequences
            ...
            if (PySequence_Check(first)){
                SAM_error error = new_error();
                SAM_set_matrix_t func = SAM_set_matrix_func(DAO_TK_SAM_lib_handle, tech, group, name, &error);
            ...
            // array
                ...
                SAM_error error = new_error();
                SAM_set_array_t func = SAM_set_array_func(DAO_TK_SAM_lib_handle, tech, group, name, &error);
            ...
            SAM_error error = new_error();
            SAM_set_table_t func = SAM_set_table_func(DAO_TK_SAM_lib_handle, tech, group, name, &error);
...
static int PySAM_load_defaults(PyObject* self, PyObject* x_attr, void* data_ptr, char* cmod, char* def){
    ...
    sprintf(path, "%sdefaults/%s_%s.df", DAO_TK_SAM_lib_dir, cmod_lower, def_lower);
    free(def_lower);
    free(cmod_lower);
